...
class Metric_SSC():
    def __init__(self,
        ...
        self.class_names = ['others','barrier','bicycle','bus','car',
    ...
    def count_miou(self):
        ...
        if self.logger is not None:
            self.logger.info(f'===> per class IoU of {self.cnt} samples:')
            for ind_class in range(self.num_classes-1):
                self.logger.info(f'===> {self.class_names[ind_class]} - IoU = ' + str(round(iou_ssc[ind_class], 2)))
            
            self.logger.info(f'===> mIoU of {self.cnt} samples: ' + str(round(np.nanmean(iou_ssc[:self.num_classes-1]), 2)))
            self.logger.info(f'===> IoU of {self.cnt} samples: ' + str(round(iou, 2)))
            self.logger.info(f'===> Precision of {self.cnt} samples: ' + str(round(precision * 100, 2)))
            self.logger.info(f'===> Recall of {self.cnt} samples: ' + str(round(recall * 100, 2)))
